...
public class HDFSWriter implements AutoCloseable{
    ...
    public HDFSWriter(Config config, RecordOffsetObject lastObject) throws IOException {
        ...
        if (useMockKafkaConsumer) {
            // TODO: CODE FOR TEST-MODE GOES HERE! When test-mode is on, use normal HDFS access instead of Kerberized access. The target HDFS server should also be set to be the test-server.
    ...
    public void commit(File syslogFile) {
        ...
        if (useMockKafkaConsumer) {
            // TODO: CODE FOR TEST-MODE GOES HERE!
        }else {
